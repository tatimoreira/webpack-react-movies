...
const PROPS = Array.from(prop_types_1.default.keys());
/**
 *
...
const searchProps = (answers, input) => {
    input = input || "";
    return Promise.resolve(PROPS.filter((prop) => prop.toLowerCase().includes(input.toLowerCase())));
};
/**
 *
...
class AddGenerator extends Generator {
    ...
    prompting() {
        ...
        let action;
        const self = this;
        const manualOrListInput = (promptAction) => webpack_scaffold_1.Input("actionAnswer", `What do you want to add to ${promptAction}?`);
        let inputPrompt;
        // first index indicates if it has a deep prop, 2nd indicates what kind of
        const isDeepProp = [false, false];
        return this.prompt([
            webpack_scaffold_1.AutoComplete("actionType", "What property do you want to add to?", {
                pageSize: 7,
                source: searchProps,
                suggestOnly: false,
            }),
            ...
            let defOrPropDescription = webpackSchemaProp
                ...
                : webpackSchema.properties[action].properties
                    ...
                    : webpackSchema.properties[action].anyOf
                        ? webpackSchema.properties[action].anyOf.filter((p) => p.properties || p.enum)
                        : null;
            if (Array.isArray(defOrPropDescription)) {
                // Todo: Generalize these to go through the array, then merge enum with props if needed
                const hasPropertiesProp = traverseAndGetProperties(defOrPropDescription, "properties");
                const hasEnumProp = traverseAndGetProperties(defOrPropDescription, "enum");
            ...
            const webpackDevserverSchemaProp = action === "devServer" ? webpackDevServerSchema : null;
            ...
            */
            if (action === "plugins") {
                const pluginExist = glob
                    ...
                    .pop()
                    .replace(".js", ""))
                    .find((p) => p.toLowerCase().indexOf(answerToAction.actionAnswer) >= 0);
                if (pluginExist) {
                    ...
                    if (pluginsSchemaPath) {
                        const constructorPrefix = pluginsSchemaPath.indexOf("optimize") >= 0
                            ? "webpack.optimize"
                            : "webpack";
                    ...
                    npm_exists_1.default(answerToAction.actionAnswer)
                        .then((p) => {
                        if (p) {
                            this.dependencies.push(answerToAction.actionAnswer);
                            const normalizePluginName = answerToAction.actionAnswer.replace("-webpack-plugin", "Plugin");
                            const pluginName = replaceAt(normalizePluginName, 0, normalizePluginName.charAt(0).toUpperCase());
                ...
                if (isDeepProp[0]) {
                    ...
                    this.prompt([
                        webpack_scaffold_1.Input("deepProp", actionMessage),
                    ]).then((deepPropAns) => {
                        ...
                        if (isDeepProp[1] === "other") {
                            ...
                            this.prompt([
                                webpack_scaffold_1.Input("innerProp", othersDeepPropKey),
                            ]).then((innerPropAns) => {
