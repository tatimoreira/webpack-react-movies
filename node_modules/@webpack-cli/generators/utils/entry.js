 ...
 */
function entry(self, answer) {
    let entryIdentifiers;
    let result;
    if (answer.entryType) {
        result = self
            .prompt([
            webpack_scaffold_1.InputValidate("multipleEntries", "Type the names you want for your modules (entry files), separated by comma [example: app,vendor]", validate_1.default),
        ])
            ...
            return forEachPromise(entryIdentifiers, (entryProp) => self.prompt([
                webpack_scaffold_1.InputValidate(`${entryProp}`, `What is the location of "${entryProp}"? [example: ./src/${entryProp}]`, validate_1.default),
        ...
        result = self
            .prompt([
            webpack_scaffold_1.InputValidate("singularEntry", "Which module will be the first to enter the application? [default: ./src/index]"),
        ])
